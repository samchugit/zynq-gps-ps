...
static int parity(uint8_t *p, uint8_t *word, uint8_t D29, uint8_t D30)
{
    ...
    for (int i = 1; i < 25; i++)
        d[i] ^= D30;
    p[0] = D29 ^ d[1] ^ d[2] ^ d[3] ^ d[5] ^ d[6] ^ d[10] ^ d[11] ^ d[12] ^
           d[13] ^ d[14] ^ d[17] ^ d[18] ^ d[20] ^ d[23];
    p[1] = D30 ^ d[2] ^ d[3] ^ d[4] ^ d[6] ^ d[7] ^ d[11] ^ d[12] ^ d[13] ^
           d[14] ^ d[15] ^ d[18] ^ d[19] ^ d[21] ^ d[24];
    p[2] = D29 ^ d[1] ^ d[3] ^ d[4] ^ d[5] ^ d[7] ^ d[8] ^ d[12] ^ d[13] ^
           d[14] ^ d[15] ^ d[16] ^ d[19] ^ d[20] ^ d[22];
    p[3] = D30 ^ d[2] ^ d[4] ^ d[5] ^ d[6] ^ d[8] ^ d[9] ^ d[13] ^ d[14] ^
           d[15] ^ d[16] ^ d[17] ^ d[20] ^ d[21] ^ d[23];
    p[4] = D30 ^ d[1] ^ d[3] ^ d[5] ^ d[6] ^ d[7] ^ d[9] ^ d[10] ^ d[14] ^
           d[15] ^ d[16] ^ d[17] ^ d[18] ^ d[21] ^ d[22] ^ d[24];
    p[5] = D29 ^ d[3] ^ d[5] ^ d[6] ^ d[8] ^ d[9] ^ d[10] ^ d[11] ^ d[13] ^
           d[15] ^ d[19] ^ d[22] ^ d[23] ^ d[24];
...
void CHANNEL::DataFetch()
{
    ...
    Debug("DataFetch rx_state: {}, rx_state_last: {}.", rx_state,
          rx_state_last);
...
void CHANNEL::BitSync()
{
    ...
    Debug("BitSync edge_total:{}, max_edge_num:{}, sec_edge_num:{}.",
    ...
    if (edge_total >= BIT_SYNC_MAX && max_edge_num >= BIT_SYNC_HIGH &&
        sec_edge_num <= BIT_SYNC_LOW) {
        bit_sync_ok = 1;
...
uint16_t CHANNEL::ParityCheck(uint8_t *buf, uint16_t *nbits)
{
    ...
    // Upright or inverted preamble, setting of parity bits resolves phase
    // ambiguity.
...
void CHANNEL::Service()
{
    ...
    for (int watchdog = 0; watchdog < TIMEOUT; watchdog++) {
        ...
        if (data_fetch_ok == 1) {
            BitSync();
            if (bit_sync_ok == 1) {
                Info("Bit synced for channel {}: PRN {}. Bit offset {}.", ch,
                     sv, bit_head);
                BitSampling();
